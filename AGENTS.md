...
The user docs state the rules. These are the reasons behind them, for development work.

- **The `:ecto` insert checks for an already-inserted struct before the `before_insert` hooks**, so
  it fails before hooks have side effects. `Ecto.put_meta(struct, state: :built)` or an `insert:`
  capture opts out.
- **`insert_via:` targets run no hooks and no already-inserted check.** A target is the user's
  function, so composition happens inside it, and a target often receives an already-inserted
  struct (insert a row, then index it).
- **Hooks are empty by default, and no association-reset hook is shipped.** Root default hooks
  would change what `:replace_parent` means on a root module. The reset is a Cookbook recipe.
- **`disable:` uses `true` for off, and its resolved value holds only the disabled keys.** The
  common case names what is off, and reflection reads as "what is off here". `false` on a key
  that is not disabled is a no-op, since the allowed-key list already catches typos.
  `:non_struct_list` is the one key not named after a function, because "non-struct factory" is
  the docs' term.
- **A variant without a body merges `defaults:`/`force:` before resolving `assocs:`.** Otherwise a
  `defaults:` value for a declared key would be dead code: the builder would run on the absent key,
  and the built struct would win the merge.
- **Non-field keys are checked before `struct!/2`, not dropped.** Dropping allowed non-field keys
  automatically would hide what the body returns; the check names the keys and the fix instead.
- **Rejected:** ExMachina-style strategy families (a pipe into `insert_*_struct` is shorter); a
  store behaviour (an arity-2 capture covers it); bulk `insert_all` (autogenerated fields,
  cascades, and hooks behave differently); `build_stubbed` (fake IDs pass
  changesets and fail foreign-key constraints); process-scoped sequences (uniqueness needs a
  global counter); a Plug-style stage pipeline (hooks with placements already are the middleware
  stack).

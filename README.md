...
it to a clean map for changesets or controller tests. Ecto metadata is stripped for Ecto
schemas; plain structs are converted with `Map.from_struct/1`:
...
foreign key is set automatically. Nil values are preserved.
...
build_*_params (calls build_*_struct internally):

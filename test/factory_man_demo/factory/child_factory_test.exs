defmodule FactoryManDemo.Factory.ChildFactoryTest do
  ...
  describe "associations" do
    ...
    test "caller can provide association params (built via assocs:)" do
    ...
    test "an association of the wrong struct type raises" do
      assert_raise ArgumentError,
                   ~r/association :user in factory :author in .*ChildFactory expects a %FactoryManDemo.Users.User\{\}/,
                   fn ->
  ...
  describe "list functions" do
    ...
    test "params_list preserves caller-provided values" do
      result = ChildFactory.build_user_params_list(1, %{username: "test", first_name: "John"})

      assert [%{username: "test", first_name: "John"}] = result
    end
  end
  ...
  describe "factory options" do
    ...
    test "body: :struct still generates params functions derived from the struct" do
      ...
      params = ChildFactory.build_raw_user_params()
  ...
  describe "parameter patterns" do
    ...
    test "pattern match without default — requires specific keys" do
      assert_raise UndefinedFunctionError, fn ->
        apply(ChildFactory, :build_no_default_fallback, [])
      end

      assert_raise FunctionClauseError, fn ->
        apply(ChildFactory, :build_no_default_fallback, [%{other: "data"}])
      end

      params = ChildFactory.build_no_default_fallback(%{username: "test"})
      assert params.username == "test"
    end
    ...
    test "nested pattern raises on non-matching input" do
      assert_raise FunctionClauseError, fn ->
        apply(ChildFactory, :build_nested_pattern, [%{other: "data"}])
      end
    end

    test "deep nested pattern match works with multiple levels" do
      params = ChildFactory.build_deep_nested(%{config: %{nested: %{value: "test123"}}})
  ...
  describe "build_*_params (derived from struct)" do
    ...
    test "removes belongs_to association structs" do
      params = ChildFactory.build_author_params()
    ...
    test "sets FK for persisted belongs_to" do
      user = ChildFactory.insert_user()
      params = ChildFactory.build_author_params(%{user: user})
  ...
  describe "build_*_string_params" do
    test "returns string keys" do
      params = ChildFactory.build_user_string_params()
    ...
    test "accepts overrides" do
      params = ChildFactory.build_user_string_params(%{username: "bob"})

      assert params["username"] == "bob"
    end

    test "leaves struct values untouched" do
      params = ChildFactory.build_user_string_params(%{created_at: ~U[2026-01-01 00:00:00Z]})
